...
var ex_gained = enemy.exp_gained
...
var starting_player_str = pStrength
var starting_player_mag = pMagic



func start():
	_FileData = world.FileData
	await up_stats()
...
func enemy_turn():
	...
	if is_defending:
		...
		await $AnimationPlayer.animation_finished
		await (get_tree().create_timer(1.4).timeout) # animation for player taking damage
...
func _on_attack_1_pressed():
	...
	if not randomValue == 1:
		...
		await (get_tree().create_timer(1.4).timeout) # timer for player animation attacking
...
func OPdamage(times_hit):
	current_enemy_health = max(0, current_enemy_health - round(pDamage * (1 + (pStrength * (times_hit * 0.1)))))
	set_health($EnemyContainer/ProgressBar, current_enemy_health, enemy.health)
	$AnimationPlayer.play("enemy_damaged")
	await $AnimationPlayer.animation_finished
	qt_eevent.hide()
